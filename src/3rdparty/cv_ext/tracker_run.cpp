...
TrackerRun::~TrackerRun() {
	...
	std::cout << "min,"
			<< *std::min_element(this->frameTime.begin(), this->frameTime.end())
			<< std::endl;
	std::cout << "min,"
			<< *std::max_element(this->frameTime.begin(), this->frameTime.end())
			<< std::endl;
	std::cout << "mean,"
			<< std::accumulate(this->frameTime.begin(), this->frameTime.end(),
					0.0) / static_cast<double>(this->frameTime.size())
			<< std::endl;
}
...
//输出结果和时间
	std::string outputFilePath = "/home/liguanqun/resultsFile.txt";
...
bool TrackerRun::update() {
	...
	if (_frameIdx == 1) {
		...
		cv::resize(_image[0], resized[0],
				cv::Size_<double>(_image[0].cols / SCALE,
						_image[0].rows / SCALE), 0, 0, cv::INTER_LINEAR);
		cv::resize(_image[1], resized[1],
				cv::Size_<double>(_image[1].cols / SCALE,
						_image[1].rows / SCALE), 0, 0, cv::INTER_NEAREST);
		cv::Rect_<double> r(_boundingBox.x / SCALE, _boundingBox.y / SCALE,
	...
	this->frameTime.push_back(
			static_cast<double>(tDuration)
	...
	image_2 = image_2 * 8;
	cv::rectangle(image_2, _boundingBox.tl(), _boundingBox.br(),
			cv::Scalar(65536));
	...
	std::cout << "the current overlap is " << overlap
			<< "   and the sum of it is  " << _overlap_sum << std::endl;
	...
	ss.clear();
	ss << "#" << _frameIdx;
	putText(hudImage, ss.str(), cv::Point(hudImage.cols - 60, 20),
	...
	outfile << _boundingBox.x << "," << _boundingBox.y << ","
...
void TrackerRun::printResults(const cv::Rect_<double>& boundingBox,
		bool isConfident, bool isTracked) {
	if (_resultsFile.is_open()) {
		if (boundingBox.width > 0 && boundingBox.height > 0 && isConfident
			...
			_resultsFile << boundingBox.x << "," << boundingBox.y << ","
					...
					<< boundingBox.height + boundingBox.y << "," << 0
					<< std::endl;
		} else {
...
void TrackerRun::printResultsTiming(
		const std::vector<int64> &singleFrameTiming) {
	if (_resultsFileTime.is_open()) {
		...
		for (int i = 0; i < (int) singleFrameTiming.size(); i++) {
			...
			if (singleFrameTiming[i] != 0)
				elapsedTime = static_cast<double>(singleFrameTiming[i])
						/ fticks;
...
float TrackerRun::Overlap(const cv::Rect_<double>& box1,
		const cv::Rect_<double>& box2) {
